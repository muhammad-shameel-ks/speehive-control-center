...
export function Ms365MailButton({ searchParams }: Props) {
  ...
  useEffect(() => {
    fetch("/api/ms365/config", { cache: "no-store" })
      ...
      .then((data: ConfigResponse) => {
        ...
        else if (!data.connections["outlook-mail"]) setConnection("unauthorized");
  ...
  async function onClick() {
    ...
    setResponse(null);
    try {
      const res = await fetch("/api/ms365/outlook-mail", { cache: "no-store" });
