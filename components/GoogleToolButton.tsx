...
export function GoogleToolButton({ serverId, label, searchParams }: Props) {
  ...
  useEffect(() => {
    fetch("/api/google/config", { cache: "no-store" })
      ...
      .then((data: ConfigResponse) => {
        ...
        else if (!data.connections[serverId]) setConnection("unauthorized");
  ...
  function onSignIn() {
    window.location.href = `/api/google/login?server=${serverId}`;
  }
  ...
  async function onClick() {
    ...
    setResponse(null);
    try {
      const res = await fetch(`/api/google/${serverId}`, { cache: "no-store" });
  ...
  const helpText = (() => {
    ...
    if (connection === "unconfigured")
      return "Google credentials are not configured on this server.";
    if (connection === "unauthorized")
      return "Click to sign in to this Google service.";
    return "";
  })();
